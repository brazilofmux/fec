...
void RS_DECODER_T1::calculate_syndromes(const GF recd[nn], std::vector<GF>& syndromes) {
    syndromes.assign(2 * 1 + 1, 0);

    for (int j = 0; j < nn; j++) {
        if (recd[j] != 0) {
            const int log_recd = poly2pow_[recd[j]];
            syndromes[1] ^= pow2poly_[mod_nn_full(j * b0_ + log_recd)];
            syndromes[2] ^= pow2poly_[mod_nn_full(j * (b0_ + 1) + log_recd)];
        }
    }

    syndromes[1] = poly2pow_[syndromes[1]];
    syndromes[2] = poly2pow_[syndromes[2]];
}
