...
void RS_ENCODER::RSGenTable(void)
{
    ...
    {
        const int feedback = Poly2Pow[ch];
#ifdef SYMMETRICAL_GENERATOR
#ifdef SYMMETRICAL_ENCODE
        for (j = 0; j < tt; j++)
#else
...
void RS_ENCODER::RSEncodeGeneral(GF data[MAX_KK], GF bb[2*MAX_TT])
{
    ...
    if (2 <= this->tt)
    {
        ...
        else if (2 == this->tt)
        {
            ...
            while (cnt >= 4)
            {
                LFSR ^= *reinterpret_cast<UINT32*>(data + i);
                LFSR ^= (reinterpret_cast<UINT32*>(this->ptable)[static_cast<GF>(LFSR)]) << 8;
                LFSR = ROTR32(LFSR, 8);
                LFSR ^= (reinterpret_cast<UINT32*>(this->ptable)[static_cast<GF>(LFSR)]) << 8;
                LFSR = ROTR32(LFSR, 8);
                LFSR ^= (reinterpret_cast<UINT32*>(this->ptable)[static_cast<GF>(LFSR)]) << 8;
            ...
            *reinterpret_cast<UINT32*>(bb) = LFSR;
            while (cnt)
            {
                const GF feedback = bb[0] ^ data[i];
    ...
    else if (1 == this->tt)
    {
        ...
        while (cnt >= 2)
        {
            LFSR ^= *reinterpret_cast<USHORT*>(data + i);
            LFSR ^= (this->ptable)[2 * static_cast<GF>(LFSR)] << 8;
            LFSR ^= (this->ptable)[2 * static_cast<GF>(LFSR >> 8)];
            cnt -= 2;
            i += 2;
        }
        *reinterpret_cast<USHORT*>(bb) = LFSR;
        if (cnt)
        {
            const GF feedback = bb[0] ^ data[i];
...
void RS_ENCODER::RSEncode32(GF data[MAX_KK], GF bb[2*MAX_TT])
{
    ...
    {
        const GF feedback = bb[0]^data[i];
        GF* TableRow = ptable + 64 * feedback;
...
void RS_ENCODER::RSEncode14(GF data[MAX_KK], GF bb[2*MAX_TT])
{
    ...
    {
        const GF feedback = bb[0]^data[i];
        GF* TableRow = ptable + 28 * feedback;
...
void RS_ENCODER::RSEncode2(GF data[MAX_KK], GF bb[2*MAX_TT])
{
    UINT32 LFSR = 0;
    int cnt = 62;
    auto pul = reinterpret_cast<UINT32*>(data);
    auto plongtable = reinterpret_cast<UINT32*>(ptable);
    while (cnt--)
    {
        LFSR ^= *pul++;
        LFSR ^= plongtable[static_cast<GF>(LFSR)] << 8;
        LFSR  = ROTR32(LFSR,8);
        LFSR ^= plongtable[static_cast<GF>(LFSR)] << 8;
        LFSR  = ROTR32(LFSR,8);
        LFSR ^= plongtable[static_cast<GF>(LFSR)] << 8;
...
{
    USHORT LFSR = 0;
    auto pus = reinterpret_cast<USHORT*>(data);
    int cnt = 126;
    while (cnt--)
    {
        LFSR ^= *pus++;
...
#else
        LFSR ^= ptable[2*static_cast<GF>(LFSR)] << 8;
        LFSR ^= ptable[2*static_cast<GF>(LFSR >> 8)];
#endif
    }
    *reinterpret_cast<USHORT*>(bb) = LFSR;

    const GF feedback = bb[0]^data[252];
#if 0
