...
RS_FLIPPED_ENCODER_T4::RS_FLIPPED_ENCODER_T4(int b0)
...
RS_FLIPPED_ENCODER_T4::~RS_FLIPPED_ENCODER_T4() {
    delete[] ptable;
}

void RS_FLIPPED_ENCODER_T4::RSGenPoly() {
    // Initially, g(x) = (X+@^b0)
    gg[0] = pow2poly_[b0_];
    gg[1] = 1;
...
void RS_FLIPPED_ENCODER_T4::RSGenTable() {
...
void RS_FLIPPED_ENCODER_T4::RSEncode(GF data[MAX_KK], GF bb[2 * MAX_TT]) {
    // Clear parity buffer
    memset(bb, 0, 2 * tt_);
