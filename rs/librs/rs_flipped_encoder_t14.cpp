...
RS_FLIPPED_ENCODER_T14::RS_FLIPPED_ENCODER_T14(int b0)
    : RS_ENCODER_BASE(14, b0)  // tt is always 14
...
RS_FLIPPED_ENCODER_T14::~RS_FLIPPED_ENCODER_T14() {
    delete[] ptable;
}

void RS_FLIPPED_ENCODER_T14::RSGenPoly() {
    // Initially, g(x) = (X+@^b0)
    gg[0] = pow2poly_[b0_];
    gg[1] = 1;
...
void RS_FLIPPED_ENCODER_T14::RSEncode(GF data[MAX_KK], GF bb[2 * MAX_TT]) {
    // Clear parity buffer
    memset(bb, 0, 2 * tt_);
    ...
    for (int i = 0; i < kk_; i++) {
        ...
        process_byte1(bb, TableRow, 24);
        process_byte1(bb, TableRow, 25);
        process_byte1(bb, TableRow, 26);
