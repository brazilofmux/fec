...
RS_ENCODER_T64::RS_ENCODER_T64(int b0)
...
RS_ENCODER_T64::~RS_ENCODER_T64() {
    delete[] ptable;
}

void RS_ENCODER_T64::RSGenPoly() {
    // Initially, g(x) = (X+@^b0)
    gg[0] = pow2poly_[b0_];
    gg[1] = 1;
...
void RS_ENCODER_T64::RSGenTable() {
...
void RS_ENCODER_T64::RSEncode(GF data[MAX_KK], GF bb[2 * MAX_TT]) {
    // Clear parity buffer
    memset(bb, 0, 2 * tt_);
