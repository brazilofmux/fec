...
RS_FLIPPED_ENCODER_T32::RS_FLIPPED_ENCODER_T32(int b0)
    : RS_ENCODER_BASE(32, b0)  // tt is always 32
...
RS_FLIPPED_ENCODER_T32::~RS_FLIPPED_ENCODER_T32() {
    delete[] ptable;
}

void RS_FLIPPED_ENCODER_T32::RSGenPoly() {
    // Initially, g(x) = (X+@^b0)
    gg[0] = pow2poly_[b0_];
    gg[1] = 1;
...
void RS_FLIPPED_ENCODER_T32::RSEncode(GF data[MAX_KK], GF bb[2 * MAX_TT]) {
    // Clear parity buffer
    memset(bb, 0, 2 * tt_);
    ...
    for (int i = 0; i < kk_; i++) {
        ...
        // Process seven 8-byte blocks using 64-bit operations
        for (int j = 0; j <= 48; j += 8) {
            process_block8(bb + j, TableRow + j);
        }

        // Process one 4-byte block
        process_block4(bb + 56, TableRow + 56);

        // Process remaining bytes individually
        process_byte1(bb, TableRow, 60);
        process_byte1(bb, TableRow, 61);
        process_byte1(bb, TableRow, 62);
