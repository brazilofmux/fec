...
RS_ENCODER_T2::RS_ENCODER_T2(int b0)
...
RS_ENCODER_T2::~RS_ENCODER_T2() {
    delete[] ptable;
}

void RS_ENCODER_T2::RSGenPoly() {
    // Initially, g(x) = (X+@^b0)
    gg[0] = pow2poly_[b0_];
    gg[1] = 1;
...
void RS_ENCODER_T2::RSEncode(GF data[MAX_KK], GF bb[2 * MAX_TT]) {
